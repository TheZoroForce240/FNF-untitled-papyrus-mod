...
import undertale.BattleMenu;
import undertale.BattleManager;
...
class PlayState extends MusicBeatState
{
	...
	public var battleMenu:BattleMenu;
	public var battleScriptName:String = "";
	public var battleCharacterName:String = "";
	public var sectionToEndAttack:Int = -1;
	...
	override public function create()
	{
		...
		if (battleScriptName != "")
		{
			...
			battleMenu = new BattleMenu(battleScriptName, battleCharacterName);
			battleMenu.cameras = [camHUD];
			battleMenu.y = 370;
			add(battleMenu);
			battleMenu.onStartAttack = function()
			{
				var maxSec = Math.floor(Conductor.getBeatRounded(songLength)/4);
				if (battleMenu.nextAttack == "")
				{
					var sec = curSection;
					if (curStep % 16 >= 8)
						sec++; //too close to end of sec so skip
					var sectionsToLoadStart = sec+1;
					var sectionsToLoadEnd = (sec - (sec%4))+4;
					if (sectionsToLoadEnd-sectionsToLoadStart <= 2)
						sectionsToLoadEnd += 4; //go another 4 sections if halfway through
					if (sectionsToLoadEnd >= maxSec)
						sectionsToLoadEnd = maxSec; //limit so it doesnt break
					sectionToEndAttack = sectionsToLoadEnd;
					loadNotesWithinSections(sectionsToLoadStart, sectionsToLoadEnd);
				}
				callOnLuas("onStartAttack", [battleMenu.nextAttack]);
			}
			clearNotesBefore(128390123890);
			canPause = false;
			vocals.volume = 0;
		}
	...
	public function updateScore(miss:Bool = false)
	{
		if (battleMenu != null)
		{
			...
			+ '  Acc: '
			+ (ratingName != '0%' ? ' ${Highscore.floorDecimal(ratingPercent * 100, 2)}%' : '');
		}
		else 
		{
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 1, battleMenu == null);
		FlxG.sound.music.pitch = playbackRate;
		if (battleMenu == null)
	...
	function loadNotesWithinSections(start:Int, end:Int)
	{
		for (s in start...end)
		{
			var section = SONG.notes[s];
			if (section != null)
			{
				for (songNotes in section.sectionNotes)
				{
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case "Start Undertale Attack": 
				if (battleManager == null)
				{
					battleManager = new BattleManager();
					add(battleManager);
					battleManager.cameras = [camHUD];
					battleManager.onDamage = function(damage:Float)
					{
						callOnLuas("onDamage", [damage]);
						health -= (damage/92)*2;
					}
				}

				var alreadyIsAttack = (battleManager.box != null && battleManager.heart != null);
				if (alreadyIsAttack)
				{
					battleManager.switchAttack(value1);
				}
				else 
				{
					battleManager.loadAttack(value1);
					battleManager.startAttack();
				}

			case "End Undertale Attack": 

				if (battleManager == null || battleManager.box == null)
					return;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		else
		{
			if (battleMenu != null)
			{
				...
				comboSpr.setGraphicSize(Std.int(comboSpr.width * 1.5));
			}
			else 
			{
	...
	override function sectionHit()
	{
		...
		if (sectionToEndAttack != -1)
		{
			if (curSection >= sectionToEndAttack)
			{
				vocals.volume = 0;
				sectionToEndAttack = -1;
				callOnLuas("onEndAttack", []);
				battleMenu.endAttack();
			}
		}
	...
	public function callOnLuas(event:String, args:Array<Dynamic>, ignoreStops = true, exclusions:Array<String> = null):Dynamic {
		...
		if (battleMenu != null)
		{
			battleMenu.script.call(event, args);
		}
