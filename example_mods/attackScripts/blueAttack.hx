...
{
    var loop = 0;
    startTimer(0.25, function(tmr:FlxTimer)
    {
        var b = makeBoneBottom(box.x+box.width, FlxG.random.int(-100, -400), FlxG.random.int(100, 200));
        b.blue = true;
        loop++;
    }, 32);

    startTimer(10, function(tmr:FlxTimer)
    {
        playSound("snd_tempbell", 1);
        heart.blue = true;
    }, 1);
    startTimer(10.7, function(tmr:FlxTimer)
    {
        heart.blue = true;
        makeBoneBottom(box.x+box.width, -250, 50);
    }, 1);
}
