...
function initAttack(box, heart)
{
    ...
    var blueSwap = true;
    doTimer(new FlxTimer().start(Conductor.stepCrochet*0.001*8, function(tmr:FlxTimer)
    {
        blueSwap = !blueSwap;
        
        var tBone = makeBone(0+10, box.y, "ut/bones/spr_s_bonestab_v", Conductor.crochet, 0, 20, 75, blueSwap, !blueSwap);

        var tBone2 = makeBone(1280-20, box.y+box.height, "ut/bones/spr_s_bonestab_v", -Conductor.crochet, 0, 20, 75, !blueSwap, blueSwap);
        tBone2.y -= tBone.height;
    }, 0));

    var l = 0;
    doTimer(new FlxTimer().start(Conductor.stepCrochet*0.001*16, function(tmr:FlxTimer)
    {
        //var iL = (0-l)+6;
        createBlaster(box.x + l*100, box.y-200, 0, 0.5, Conductor.stepCrochet*0.001*8);
        //createBlaster(box.x + iL*100, box.y+box.height+100, 180, 0.5, Conductor.stepCrochet*0.001*12);
        l++;
        if (l > 6)
            l = 0;

        //createBlaster(box.x-100, box.y+20, -90, 0.5, Conductor.stepCrochet*0.001*8);
    }, 0));
    doTimer(new FlxTimer().start(Conductor.stepCrochet*0.001*8, function(tmr:FlxTimer)
    {
        doTimer(new FlxTimer().start(Conductor.stepCrochet*0.001*16, function(tmr:FlxTimer)
        {    
            var iL = (0-l)+6;
            //createBlaster(box.x + l*100, box.y-200, 0, 0.5, Conductor.stepCrochet*0.001*8);
            createBlaster(box.x + iL*100, box.y+box.height+100, 180, 0.5, Conductor.stepCrochet*0.001*8);
            //createBlaster(box.x-100, box.y+120, -90, 0.5, Conductor.stepCrochet*0.001*8);
        }, 0));
    }));

    doTimer(new FlxTimer().start(Conductor.stepCrochet*0.001*32, function(tmr:FlxTimer)
    {
        if (heart.blueGravityDir == 0)
            heart.blueGravityDir = 2;
        else 
            heart.blueGravityDir = 0;
        heart.gravityAdd = 10;
        playSound('snd_tempbell', 0.7);
    }, 0));
