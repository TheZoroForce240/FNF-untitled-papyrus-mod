...
{
    var loop = 0;
    startTimer(0.5, function(tmr:FlxTimer)
    {
        makeBoneBottom(box.x-10, 120, 60);
        startTimer(0.75, function(tmr:FlxTimer)
        {
            makeBoneTop(box.x-10, 120, 150);
            startTimer(0.75, function(tmr:FlxTimer)
            {
                makeBoneBottom(box.x-10, 180, 30);
            }, 5);
            startTimer(0.75*6, function(tmr:FlxTimer)
            {
                makeBoneTop(box.x-10, 120, 150);
                startTimer(0.75, function(tmr:FlxTimer)
                {
    ...
    startTimer(8, function(tmr:FlxTimer)
    {
        end();
    }, 1);
}
