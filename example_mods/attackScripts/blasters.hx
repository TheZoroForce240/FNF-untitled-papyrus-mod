...
function initAttack(box, heart)
{
    ...
    doTimer(new FlxTimer().start(1, function(tmr:FlxTimer)
    {
        ...
        createBlasterAroundBox(heart.hitbox.x, heart.hitbox.y, FlxG.random.float(0, 360), 0);

        //createBlaster(box.x + FlxG.random.float(0, box.width), box.y-100, 0, 0);
        //createBlaster(heart.x, heart.y, 180, 0);
